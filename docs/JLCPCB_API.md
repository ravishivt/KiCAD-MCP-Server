...
The signature must be computed before sending. Generate `NONCE` (32 random alphanumeric chars), `TIMESTAMP` (unix seconds), and `SIGNATURE` (HMAC-SHA256 of the string-to-sign, base64-encoded), then:

```bash
curl -X POST https://open.jlcpcb.com/overseas/openapi/component/getComponentLibraryList \
  -H "Content-Type: application/json" \
  -H 'Authorization: JOP appid="<APP_ID>",accesskey="<ACCESS_KEY>",nonce="<NONCE>",timestamp="<TIMESTAMP>",signature="<SIGNATURE>"' \
  -d '{"currentPage": 1, "pageSize": 100}'
```
