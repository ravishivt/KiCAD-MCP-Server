...
parser a hand-written symbol block and by stubbing DynamicSymbolLoader / PinLocator,
so they run identically in CI and on a developer machine.
"""
...
class TestSearchSchematicSymbols:
    def test_requires_query(self):
        c = SymbolPinCommands()
        assert c.search_schematic_symbols({"query": "  "})["success"] is False


class TestGetComponentPinPositions:
    def test_requires_params(self):
        c = SymbolPinCommands()
        assert c.get_component_pin_positions({"schematicPath": "x"})["success"] is False
        assert c.get_component_pin_positions({"reference": "U1"})["success"] is False

    def test_happy_path(self, monkeypatch):
        fake = MagicMock()
        fake.get_all_symbol_pins.return_value = {"1": [10.0, 20.0], "2": [10.0, 30.0]}
        fake._get_lib_id.return_value = "Device:R"
        fake.get_symbol_pins.return_value = {"1": {"name": "~", "type": "passive"}}
        fake.get_pin_angle.return_value = 90.0
        monkeypatch.setattr(symbol_pins, "PinLocator", lambda: fake)
        c = SymbolPinCommands()
        r = c.get_component_pin_positions({"schematicPath": "/tmp/x.kicad_sch", "reference": "R1"})
        assert r["success"] is True
        assert r["count"] == 2
        p1 = next(p for p in r["pins"] if p["pin_number"] == "1")
        assert p1["position"] == {"x": 10.0, "y": 20.0}
        assert p1["pin_name"] == "~"
        assert p1["stub_direction_angle"] == 90.0

    def test_no_pins_found(self, monkeypatch):
        fake = MagicMock()
        fake.get_all_symbol_pins.return_value = {}
        monkeypatch.setattr(symbol_pins, "PinLocator", lambda: fake)
        c = SymbolPinCommands()
        r = c.get_component_pin_positions({"schematicPath": "/tmp/x.kicad_sch", "reference": "R99"})
        assert r["success"] is False
